...
unsigned int Texture_FromFile(const char *path, const string &directory, bool gamma = false);

class Model 
{
public:
    // Данные модели 
    vector<Texture_> Texture_s_loaded; // (оптимизация) сохраняем все загруженные текстуры, чтобы убедиться, что они не загружены более одного раза
    vector<Mesh> meshes;
    string directory;
...
private:
    ...
    Mesh processMesh(aiMesh *mesh, const aiScene *scene)
    {
        ...
        // Мы вводим соглашение об именах сэмплеров в шейдерах. Каждая диффузная текстура будет называться 'Texture__diffuseN',
        // где N - порядковый номер от 1 до MAX_SAMPLER_NUMBER. 
        // Тоже самое относится и к другим текстурам:
        // диффузная - Texture__diffuseN
        // отражения - Texture__specularN
        // нормали - Texture__normalN

        // 1. Диффузные карты
        vector<Texture_> diffuseMaps = loadMaterialTexture_s(material, aiTextureType_DIFFUSE, "Texture__diffuse");
        Texture_s.insert(Texture_s.end(), diffuseMaps.begin(), diffuseMaps.end());
		
        // 2. Карты отражения
        vector<Texture_> specularMaps = loadMaterialTexture_s(material, aiTextureType_SPECULAR, "Texture__specular");
        Texture_s.insert(Texture_s.end(), specularMaps.begin(), specularMaps.end());
		
        // 3. Карты нормалей
        std::vector<Texture_> normalMaps = loadMaterialTexture_s(material, aiTextureType_HEIGHT, "Texture__normal");
        Texture_s.insert(Texture_s.end(), normalMaps.begin(), normalMaps.end());
		
        // 4. Карты высот
        std::vector<Texture_> heightMaps = loadMaterialTexture_s(material, aiTextureType_AMBIENT, "Texture__height");
        Texture_s.insert(Texture_s.end(), heightMaps.begin(), heightMaps.end());
        
        // Возвращаем меш-объект, созданный на основе полученных данных
        return Mesh(vertices, indices, Texture_s);
    }

    // Проверяем все текстуры материалов заданного типа и загружам текстуры, если они еще не были загружены.
    // Необходимая информация возвращается в виде структуры Texture_
    vector<Texture_> loadMaterialTexture_s(aiMaterial *mat, aiTextureType type, string typeName)
    {
        vector<Texture_> Texture_s;
        for(unsigned int i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            bool skip = false;
            for(unsigned int j = 0; j < Texture_s_loaded.size(); j++)
            {
                if(std::strcmp(Texture_s_loaded[j].path.data(), str.C_Str()) == 0)
                {
                    Texture_s.push_back(Texture_s_loaded[j]);
            ...
            {   // если текстура еще не была загружена, то загружаем её
                Texture_ Texture_;
                Texture_.id = Texture_FromFile(str.C_Str(), this->directory);
                Texture_.type = typeName;
                Texture_.path = str.C_Str();
                Texture_s.push_back(Texture_);
                Texture_s_loaded.push_back(Texture_); // сохраняем текстуру в массиве с уже загруженными текстурами, тем самым гарантируя, что у нас не появятся без необходимости дубликаты текстур
            }
        }
        return Texture_s;
    }
...
unsigned int Texture_FromFile(const char *path, const string &directory, bool gamma)
{
    ...
    {
        std::cout << "Texture_ failed to load at path: " << path << std::endl;
        stbi_image_free(data);
    }
